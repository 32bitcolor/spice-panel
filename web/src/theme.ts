...
export const THEMES: { id: ThemeId, label: string, swatch: string }[] = [
  ...
  { id: 'atreides', label: 'Atreides', swatch: '#33b572' },
...
const OVERRIDE_KEYS = [
  '--accent', '--accent-foreground', '--accent-secondary', '--focus', '--link',
...
const PALETTES: Record<ThemeId, Record<string, string>> = {
  spice: {},
  atreides: {
    '--accent': '#33b572', '--accent-foreground': '#06140c', '--accent-secondary': '#d9a43b',
    '--focus': '#52d68f', '--link': '#52d68f',
  ...
  harkonnen: {
    '--accent': '#cf2b2b', '--accent-foreground': '#ffffff', '--accent-secondary': '#9aa3ad',
    '--focus': '#e85555', '--link': '#e85555',
  ...
  fremen: {
    '--accent': '#2f88c9', '--accent-foreground': '#06121c', '--accent-secondary': '#8ad8ff',
    '--focus': '#4aa6e8', '--link': '#4aa6e8',
...
export function applyTheme(id: ThemeId): void {
  ...
  // Structural theming hook: per-house CSS (HUD corner ornaments etc.) keys off
  // [data-theme="…"] in index.css — colours alone can't change shapes.
